...
def welcom_main_screen():
    p_x = int(SCREEN_WIDTH / 5)
    p_y = int((SCREEN_HEIGHT - game_image["player"].get_height())/2)

    msg_x = int((SCREEN_HEIGHT - game_image["message"].get_width())/2)
...
def main_game_play():
    ...
    while True:
        ...
        p_middle_postos = p_x + game_image["player"]. get_width()/ 2

        for pipe in up_pipe:
           pip_middle_position = pipe["x"] + game_image["pipe"][0].get_width() / 2

           if pip_middle_position <= pip_middle_position < pip_middle_position + 4:
               score += 1
            ...
            if 0<up_pipe[0]["x"] < 5:
                ...
                for pip_upper, pip_lover in zip(up_pipe, low_pipe):
                    ...
                    for diget in d:
                        ...
                        Xoffset += game_image["numbers"][diget].get_width()
